...

{
	...
	my $cd = $schema->resultset('CD')->create({ cdid => 187, artist => 1, title => 'how could hell be any worse?', year => 1982, genre => undef });

	ok(!defined($cd->get_column('genre')), 'genre is NULL');  #no accessor was defined for this column
...
{
	my $artist = $schema->resultset('Artist')->create({ artistid => 666, name => 'bad religion' });
	my $genre = $schema->resultset('Genre')->create({ genre => 88, name => 'disco' });
